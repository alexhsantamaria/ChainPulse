...
**CORREGIDO 2026-09-24 — ya no hay URLs prefirmadas de subida/descarga.** El diseño original de esta sección asumía que el navegador sube el CSV directo a R2 con una URL PUT prefirmada. Alex confirmó el 2026-09-24 (`docs/ADR/0006-cifrado-r2.md`) que el CSV se cifra en el **servidor** antes de subir — eso significa que el archivo en claro pasa por un Route Handler de Next.js, nunca directo del navegador a R2, así que ya no hace falta (ni tiene sentido) una URL PUT prefirmada. Misma lógica para la descarga: el servidor descifra y transmite el CSV al usuario ya autenticado, nunca una URL GET firmada. Checklist actualizado:

**Código ya construido (`src/infra/storage/`, sin depender de credenciales reales todavía):**
- [x] `cifradoObjeto.ts` — envelope encryption AES-256-GCM (DEK por archivo + clave maestra versionada para rotación + AAD por empresa/importación) — ver ADR-0006 para el diseño completo. 19 pruebas, sin red.
- [x] `almacenamiento.ts` — interfaz `ClienteAlmacenamiento` (subir/descargar/eliminar/existe) + `construirClaveObjetoCifrado(empresaId, importId)` (clave fija `empresas/{empresaId}/imports/{importId}/cifrado.bin` — nunca el nombre de archivo original del usuario).
- [x] `almacenamientoMemoria.ts` — implementación en memoria de `ClienteAlmacenamiento`, para probar el flujo completo de cifrado+persistencia sin R2 real (pedido explícito de Alex mientras prepara las credenciales). 7 pruebas.
- [x] `r2.ts` — implementación real (`@aws-sdk/client-s3`, instalado). Sin URLs prefirmadas: expone `subirObjeto`/`descargarObjeto`/`eliminarObjeto`/`existeObjeto`, todo server-to-R2. Solo probable con credenciales reales — mismo criterio "Windows-only" que `test:integration` de Neon.
- [x] Migración `20260924050000_incremento4_r2_cifrado_importacion_csv` — agrega `cifradoVersion`/`cifradoClaveId`/`cifradoDek`/`cifradoDekIv`/`cifradoDekAuthTag` a `ImportacionCsv` (metadata de la DEK envuelta, nunca el contenido ni la DEK en claro). **Aplicada y validada en Windows contra Neon real el 2026-09-24:** `prisma migrate dev` la aplicó limpia, `prisma validate` OK, `typecheck`/`lint` limpios, `test` 284/284 y `test:integration` 43/43 (mismos 4 archivos de siempre, incluidas las 18 pruebas de RLS/aislamiento) en verde.
- [x] `src/domain/limitesImportacionCsv.ts` — `MAX_TAMANO_ARCHIVO_BYTES` bajado de 5 MB a 4 MB: el CSV ahora pasa por el body del Route Handler (para poder cifrarlo antes de subir), sujeto al límite de Vercel Hobby (4.5 MB) — ver el comentario de cabecera de ese archivo y ADR-0006 para el detalle completo del porqué.

**Pendiente de Alex, en su cuenta de Cloudflare (yo no puedo entrar a ese dashboard) — checklist concreto:**
- [ ] Crear DOS buckets separados — uno para desarrollo/test, otro para producción (ej. `chainpulse-imports-dev` / `chainpulse-imports-prod`) — nunca el mismo bucket para ambos entornos.
- [ ] En cada bucket: dejar el acceso público (`r2.dev`) **desactivado** — el bucket debe quedar privado por defecto, sin URL pública de ningún tipo (ya no hace falta CORS tampoco, al no haber subida/descarga directa del navegador).
- [ ] Generar un API Token por bucket, con scope de **Object Read & Write limitado a ESE bucket** — nunca "Admin Read & Write" ni permisos de administración general de la cuenta (ver [permisos de R2](https://developers.cloudflare.com/r2/api/tokens/)). Dos tokens en total, uno por entorno.
- [ ] Cargar las 4 variables por entorno en Vercel (Development/Preview → bucket+token de dev; Production → bucket+token de prod) y en `.env` local: `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET_NAME` — nombres ya documentados en `.env.example`, mismo criterio de siempre: nunca pegar los valores en el chat, solo confirmar que ya están configurados.
- [ ] Generar y cargar `R2_ENCRYPTION_KEY_ACTIVA`/`R2_ENCRYPTION_KEY_ACTIVA_ID` (comando exacto en `.env.example` y en ADR-0006) — **independiente** de las credenciales de R2, y guardar un backup offline de esta clave (gestor de contraseñas, etc. — ver "Custodia" en ADR-0006, si se pierde sin backup los CSV ya cifrados quedan ilegibles para siempre).
- [ ] Regla de ciclo de vida (lifecycle rule) en cada bucket: expirar automáticamente objetos de `imports/` sin persistir a los N días (a definir con la política de retención pendiente de `MVP-DEFINITIVO.md` Sección 9) — evita acumular CSVs cifrados abandonados indefinidamente.

**Script de prueba manual contra R2 dev — ampliado de 5 a 10 pasos (2026-09-24):** `npm run r2:probar` (`scripts/probarR2.ts`) — cifra un CSV ficticio, lo sube a R2, lo descarga, lo descifra y compara byte a byte contra el original (pasos 1-4); rechazo por `empresaId` incorrecto (paso 5) y rechazo por `importId` incorrecto (paso 6, antes no existía como caso separado) — ambos deben lanzar por fallo de verificación del AAD, si no lanzan el script termina con error, es un fallo real de aislamiento; lee la clave maestra activa real vía `obtenerClaveMaestraActiva()` e imprime solo su `id` (paso 7); envuelve y desenvuelve la DEK real con esa clave maestra y verifica igualdad byte a byte (paso 8, antes el script solo usaba una DEK ad-hoc con `cifrarContenido`/`descifrarContenido`, nunca ejercitaba el envoltorio real); verifica que desenvolver la DEK rechace tanto `empresaId` como `importId` incorrectos (paso 9); borra el objeto de prueba siempre, en un `finally` (paso 10). Se niega a correr si `R2_BUCKET_NAME` no contiene "dev" en el nombre (salvo `--forzar` explícito) — única protección real contra correrlo sin querer contra producción. Nunca imprime credenciales ni la clave de cifrado, solo el `id` de la clave maestra activa, el nombre del bucket (no es secreto) y resultados OK/FALLO por paso.

**Corregida la carga de `.env` (Alex encontró que `npm run r2:probar` no cargaba las variables solo):** nuevo `scripts/_cargarEnv.ts`, importado como primera línea en `probarR2.ts` y retrofitteado en los otros tres scripts de mantenimiento (`bootstrapPgBoss.ts`, `limpiarTenantsPrueba.ts`, `limpiarCadenasDuplicadas.ts`) para que todos se comporten igual. Usa `process.loadEnvFile()` nativo de Node (soportado en el Node 22 del proyecto), ignora `ENOENT` si `.env` no existe y relanza cualquier otro error. `npm run r2:probar` ya debería funcionar como comando plano, sin necesitar `node --env-file=.env --import tsx scripts/probarR2.ts`.

**Validado por Alex contra R2 dev real el 2026-09-24 (versión de 5 pasos, antes de esta expansión):** subida, descarga, descifrado, comparación y limpieza OK; rechazo por empresa incorrecta OK. Pendiente que confirme la versión de 10 pasos completa (rechazo por `importId`, clave maestra real, envoltorio/desenvoltorio de la DEK) y el arranque como `npm run r2:probar` sin `--env-file`. `typecheck`/`lint` limpios en los 5 scripts tocados (solo los 5 falsos positivos conocidos de `Prisma.InputJsonValue`, sin relación con este cambio).

**Todavía sin construir (depende de que exista al menos el bucket de desarrollo para probar de punta a punta):**
- [ ] Route Handler de subida (`POST /api/chains/:id/imports`) — recibe el CSV en el body (≤ `MAX_TAMANO_ARCHIVO_BYTES`), lo cifra (`cifradoObjeto.ts`) y lo sube (`ClienteAlmacenamientoR2.subirObjeto`) en la misma operación.
- [ ] Route Handler de descarga/confirmación — descifra server-side y nunca expone el CSV en claro por una URL, solo al usuario ya autorizado del tenant dueño.
...
**CORREGIDO 2026-09-24 — ya no hay URL prefirmada de subida (ver ADR-0006 y "Setup concreto de Cloudflare R2" más arriba, ya actualizado con el detalle completo).** El CSV se cifra en el servidor antes de subir, así que el flujo de subida pasa por un Route Handler (body ≤ `MAX_TAMANO_ARCHIVO_BYTES`, hoy 4 MB), no por una URL PUT directa al bucket.

- [x] Instalado `@aws-sdk/client-s3` (`@aws-sdk/s3-request-presigner` ya NO hace falta — sin URLs prefirmadas, ver arriba). Cero vulnerabilidades nuevas (`npm audit` confirmado antes/después).
- [x] Variables en `.env.example`: `R2_ACCOUNT_ID`/`R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY`/`R2_BUCKET_NAME` + las 3 de cifrado (`R2_ENCRYPTION_KEY_ACTIVA`/`_ID`/`R2_ENCRYPTION_KEYS_ANTERIORES`).
- [x] Flujo de subida (diseño, código de bajo nivel listo): el servidor cifra (`cifradoObjeto.ts`) y sube el ciphertext con `ClienteAlmacenamientoR2.subirObjeto()` — ver ADR-0006 para el detalle completo de por qué se abandonó la URL prefirmada.
- **Dónde en el código:** `src/infra/storage/r2.ts` (cliente real), `almacenamiento.ts` (interfaz + `construirClaveObjetoCifrado`), `almacenamientoMemoria.ts` (fake para pruebas), `cifradoObjeto.ts` (envelope encryption).
- **Testeo:** 26 pruebas unitarias sin red (`__tests__/cifradoObjeto.test.ts`, `__tests__/almacenamiento.test.ts`) — construcción de la clave de objeto (nunca el nombre de archivo original del usuario, evita path traversal), contrato del cliente en memoria, y las 19 de cifrado (manipulación, clave incorrecta, acceso entre empresas, rotación). Integración real contra R2 solo puede correr con credenciales reales, mismo criterio "Windows-only" que `test:integration` de Neon — pendiente de que Alex complete el setup del bucket.
- **Riesgo de cifrado en reposo: resuelto, ver ADR-0006.** Cifrado de aplicación (AES-256-GCM) confirmado y construido, además del cifrado por defecto de R2.
...
**CONFIRMADO por Alex el 2026-09-24, con dos precisiones -- migración de schema escrita y validada en Windows contra Neon real (ver abajo).** Responde a dos cosas: (1) dónde persiste el resultado de `calcularCobertura` (hueco de schema encontrado más arriba) y (2) la corrección de Alex a la nota de idempotencia original -- "no demos por resuelto el problema... únicamente con un `upsert`", distinguiendo unicidad de idempotencia real.


**Confirmación de Alex (2026-09-24) -- las 4 preguntas, todas con la opción recomendada, MÁS dos precisiones sobre la propuesta original que sí cambian el diseño:**

1. **Normalización de SKU/ubicación -- corregida.** Confirmado: mayúsculas + trim de espacios en los extremos. Corrección: la propuesta original decía "colapsar espacios internos" también -- Alex lo descartó explícitamente, se preservan intactos (ceros iniciales, guiones, espacios internos, todo salvo mayúsculas/trim de extremos). Además: se guarda el valor ORIGINAL sin normalizar aparte (`skuOriginal`/`ubicacionOriginal`, solo para mostrar/auditar, nunca para comparar). Si dos filas de un mismo archivo normalizan a la misma clave pero traen datos distintos, el importador debe **reportarlo como duplicado, nunca fusionarlas en silencio** -- el índice único parcial de manual/pegado ya lo detecta a nivel de base.
2. **Alcance de una corrección -- corregido y ampliado.** Confirmado: rango de fechas explícito, nunca inferido. Corrección: un rango de fechas **por sí solo no autoriza a retirar nada** -- el alcance también exige declarar las **ubicaciones incluidas** explícitamente (`ImportacionCsv.alcanceUbicaciones`). Además, dos operaciones distintas y explícitas: **carga parcial** (los SKU/ubicaciones ausentes del archivo no se tocan, es el default) vs. **reemplazo completo del alcance declarado** (los ausentes dentro del alcance se retiran, conservando su historial). La UI de confirmación (todavía sin construir) debe mostrar una previsualización de registros nuevos/reemplazados/retirados antes de confirmar -- pendiente de diseño cuando exista Bloque B con R2.
3. **Historial para los otros 9 KPIs -- corregido, más estricto de lo que se había entendido.** Confirmado: el modelo completo de historial (vigente/reemplazadaPorId) es solo para Cobertura. Corrección importante: para los 9 KPIs **no se acepta un upsert que sobrescriba en silencio** -- se exige idempotencia real: mismo contenido devuelve el resultado existente (no-op), contenido distinto sobre una clave ya existente genera un **conflicto explícito** (nunca se sobrescribe). Esto **no requiere ninguna migración** sobre `ObservacionKpi` (que ya está aplicada) -- es un contrato del futuro código del importador/carga manual: `insert` intentando la clave única existente, y ante choque, comparar `valor`/`numerador`/`denominador` contra la fila existente -- igual reporta no-op o conflicto según corresponda, en vez de `prisma.observacionKpi.upsert(...)`. Queda anotado como contrato de escritura pendiente de implementar cuando se construya ese código, sin tocar el schema ya migrado.
4. **Versión de fórmula -- confirmado, con una precisión.** Sin reproceso retroactivo automático al subir `RULE_VERSION_KPIS` -- las filas vigentes quedan con la versión con la que se calcularon. Precisión: un recálculo futuro, si hiciera falta, será una operación **explícita y trazable que preserve el resultado anterior** -- para Cobertura eso es simplemente "otra corrección" por el mismo mecanismo de `vigente`/`reemplazadaPorId` ya diseñado; para los 9 KPIs seguiría el mismo contrato de idempotencia/conflicto explícito del punto 3, nunca un `UPDATE` silencioso.

**Migración de schema escrita:** `prisma/migrations/20260924040000_incremento4_kpis_bloque_b_cobertura/migration.sql` -- `ImportacionCsv` (genérica por KPI, hoy solo la usa Cobertura) + `ObservacionCobertura`, con los campos `skuOriginal`/`ubicacionOriginal`, `estrategia`/`alcanceFechaCorteInicio`/`alcanceFechaCorteFin`/`alcanceUbicaciones` en `ImportacionCsv`, un `CHECK` (`chk_importacion_csv_alcance_reemplazo`) que exige los tres campos de alcance juntos cuando `estrategia=REEMPLAZO_ALCANCE`, los dos índices únicos parciales de idempotencia (manual/pegado por clave de negocio, CSV por `importId+numeroFila`), un índice compuesto con nombre explícito (`idx_observaciones_cobertura_vigente_lookup`) para evitar el mismo riesgo de truncamiento a 63 caracteres que ya causó un problema real con `curadorId`, RLS + políticas + grants siguiendo el patrón ya establecido. `ObservacionCobertura`/`ImportacionCsv` agregadas a `TENANT_SCOPED_MODELS`. Escrita a mano (red bloqueada a `binaries.prisma.sh` en el entorno de Claude, `prisma validate`/`prisma migrate dev` no pueden correr acá). **Aplicada y validada en Windows contra Neon real el 2026-09-24:** `npm run prisma:migrate` aplicó `20260924040000_incremento4_kpis_bloque_b_cobertura` limpio, sin el prompt interactivo de drift que sí salió con la migración de Bloque A -- el `map:` explícito agregado al índice largo evitó ese riesgo desde el primer intento. `npx prisma validate` -> "The schema at prisma\schema.prisma is valid". `typecheck`/`lint` en cero errores, ni siquiera los 5 falsos positivos ya conocidos del stub de Prisma en Mac (Windows genera el cliente real). `npm run test` 258/258 y `npm run test:integration` 43/43 en verde (incluye `aislamientoMultitenant.integration.test.ts`, 18 pruebas de RLS/aislamiento por tenant -- mismo modelo de seguridad del que dependen `ObservacionCobertura`/`ImportacionCsv`). Sin código de importador todavía -- eso sigue esperando las credenciales de R2.
...
- [x] **Cifrado de Cloudflare R2 — confirmado por Alex el 2026-09-24: cifrado adicional a nivel de aplicación, antes de subir el archivo.** Ver ADR-0006 (`docs/ADR/0006-cifrado-r2.md`) para el diseño completo (envelope encryption AES-256-GCM, ya construido en `src/infra/storage/`, 26 pruebas verdes).
...
#### E. Cifrado de Cloudflare R2 — confirmado y construido (2026-09-24)

**Confirmado por Alex: cifrado adicional a nivel de aplicación, antes de subir el archivo, además del cifrado por defecto de R2.** Diseño completo y decisión formal en `docs/ADR/0006-cifrado-r2.md` -- esta sección queda como resumen, la fuente de verdad es el ADR.

- **Mecanismo:** envelope encryption AES-256-GCM, módulo `crypto` nativo de Node — cero dependencias nuevas. `src/infra/storage/cifradoObjeto.ts`.
- **Clave maestra CON versionado (corregido sobre la recomendación original):** `R2_ENCRYPTION_KEY_ACTIVA` + `R2_ENCRYPTION_KEY_ACTIVA_ID` + `R2_ENCRYPTION_KEYS_ANTERIORES` — a diferencia de la recomendación original (una sola `R2_ENCRYPTION_KEY` sin id), Alex pidió explícitamente poder rotar sin re-cifrar nada ya subido: cada fila guarda con qué clave se envolvió su DEK (`ImportacionCsv.cifradoClaveId`), así que rotar la clave activa nunca invalida datos existentes.
- **Flujo:** DEK aleatoria por archivo, cifra el contenido con AAD = empresaId+importId (vincula criptográficamente el archivo a su empresa/importación — pedido explícito de Alex), la propia DEK se cifra con la clave maestra activa; `(iv, authTag, dekCifrada, claveId, version)` se guarda como metadata en Postgres (`ImportacionCsv.cifradoDek*`), nunca en R2. El objeto que llega a R2 es siempre ciphertext.
- **Descarga:** un endpoint propio que descifra server-side y transmite el CSV en claro solo al usuario ya autorizado del tenant dueño — todavía sin construir (depende del resto del flujo de importación).
- **Custodia/recuperación/retención de claves:** política completa en ADR-0006 y `SEGURIDAD-credenciales.md` — sin KMS gestionado (presupuesto $0), backup offline de la clave activa es responsabilidad de Alex [HUMANO], documentado como riesgo aceptado explícito (si se pierde sin backup, los CSV ya cifrados con ella quedan ilegibles para siempre).
- **Pruebas:** 19 casos (`__tests__/cifradoObjeto.test.ts`) cubren manipulación del archivo, clave incorrecta y acceso entre empresas — los tres casos que Alex pidió probar explícitamente — más rotación de clave maestra de punta a punta.
...
## 24. Diagnóstico de los hallazgos de la Sección 23, con evidencia real (2026-09-25)

Alex compartió el detalle exacto (mensaje completo, stack trace, orden de eventos) que faltaba en la Sección 23. Diagnóstico contra el código fuente instalado, no por conjetura.

**1. P2028 ("Unable to start a transaction in the given time") en `/dashboard/eslabones`, sesión RESPONSABLE:**
- Diagnóstico: `src/infra/prisma/tenantClient.ts` ya documenta este error exacto en su propio comentario — cada consulta vía `tenantClient()` corre dentro de un `prisma.$transaction()` (para poder hacer `SET LOCAL app.tenant_id` antes de la query real) con `TX_OPTIONS_NEON = { maxWait: 15000, timeout: 20000 }` (`src/infra/prisma/tenantScope.ts`), ya aumentado sobre los defaults de Prisma específicamente para el cold-start de Neon (autosuspend). P2028 es "no se pudo ni empezar la transacción en `maxWait`" — Neon tardó más de 15s en responder a la primera conexión.
- No es un efecto del parche de `next` ni de la corrección de "Invitar responsable" — es la misma clase de problema que `TX_OPTIONS_NEON` ya fue creado para mitigar, no eliminar (el comentario de ese archivo es explícito: "no arregla el cold-start de Neon, nada del lado de la app puede evitarlo").
- **Pendiente de confirmar antes de tocar código:** si recargar la página inmediatamente después (con Neon ya despierto) funcionó sin error, esto fue un cold-start aislado y no hace falta cambiar nada. Si el P2028 se repite con la base ya despierta (conexiones subsiguientes, no la primera del día), ahí sí valdría la pena subir `maxWait`/`timeout` más allá de 15s/20s — pero no se toca sin ese dato, para no enmascarar un problema real de otro tipo (agotamiento del pool, por ejemplo) detrás de un timeout más largo.

**2. `[auth][error] UntrustedHost` en `npm run start`, y login que termina en `/` en vez de `/dashboard`:** dos causas distintas, ambas confirmadas contra el código fuente instalado (no la documentación de Auth.js, que no siempre coincide con el comportamiento real de una versión beta):
- **`UntrustedHost` — corregido en `.env.example`.** `node_modules/@auth/core/src/lib/utils/env.ts` resuelve `trustHost` con `AUTH_URL ?? AUTH_TRUST_HOST ?? VERCEL ?? CF_PAGES ?? (NODE_ENV !== "production")` — este chequeo específico **no cae a `NEXTAUTH_URL`** (a diferencia de la resolución de URL/secreto, que sí tiene ese fallback, ver Sección 23). Con `npm run start` (`NODE_ENV=production`, sin `VERCEL`) y solo `NEXTAUTH_URL` definida, `trustHost` quedaba en `false` → exactamente el error que viste. En Vercel no pasa porque la plataforma define `VERCEL=1` sola, pero cualquier `npm run start` local lo necesita explícito. `.env.example` ahora deja `AUTH_URL`/`AUTH_SECRET` descomentadas (no como alias opcional) con la cita exacta del código que lo explica.
- **Login termina en `/` — bug real, corregido.** `src/app/login/page.tsx` hacía `router.push("/")` después de un `signIn()` exitoso (sin error). El middleware (`src/middleware.ts`, `matcher: ["/activar-mfa", "/dashboard/:path*"]`) no protege `/`, así que no había ninguna segunda capa que corrigiera el destino — el login funcionaba (cookie de sesión válida, por eso entrar a `/dashboard` a mano sí funcionó), pero devolvía a la portada pública sin ninguna señal de que la sesión había quedado iniciada. Cambiado a `router.push("/dashboard")`, con el motivo documentado en el propio comentario del archivo. Válido para RESPONSABLE y ADMINISTRADOR por igual — el middleware ya redirige a `/activar-mfa` si un ADMINISTRADOR sin MFA intenta entrar a `/dashboard`, así que no hace falta bifurcar por rol acá.
- **Sobre el mismatch `127.0.0.1` vs `localhost`:** con `AUTH_URL` ahora fijada explícita, probar de nuevo con el mismo host en el navegador y en `AUTH_URL` (los dos `localhost`, o los dos `127.0.0.1`, nunca mezclados) para no introducir una variable nueva mientras se revalida.

**3. Advertencias del build — evaluadas, ninguna bloquea nada:**
- `[webpack.cache.PackFileCacheStrategy] Serializing big strings...`: aviso de rendimiento de la caché de build de Webpack (deserialización un poco más lenta en el próximo build incremental), no afecta el resultado ni el runtime. Sin acción.
- `jose`/`CompressionStream`/`DecompressionStream` no soportadas en Edge Runtime (vía `next-auth` → `@auth/core` → `jose`): mismo patrón ya visto en un intento de build en este entorno (sin red real). Es una advertencia informativa de Next sobre una API que el paquete `jose` importa condicionalmente para JWE (cifrado de JWT) — esta app usa JWT firmados (JWS) sin cifrar, así que esa rama de código no se ejecuta nunca en la práctica; el propio middleware ya está separado en `auth.config.ts` justamente para no arrastrar dependencias no aptas para Edge (ver el comentario de cabecera de ese archivo). No bloquea el build ni el arranque, confirmado por Alex ("Build... completado correctamente").
- Advertencia SSL de `pg`/`pg-connection-string` (modos `prefer`/`require`/`verify-ca` tratados como alias de `verify-full`): aviso de deprecación de la librería sobre un cambio de semántica en su v3.0.0 futura, no de esta versión. Repetida en corridas anteriores de este mismo proyecto ("Persisten... el aviso SSL", mensaje de Alex del 2026-09-24) — ya conocida, no nueva de este parche.
- Ninguna de las tres requiere acción — quedan documentadas para no re-investigarlas cada vez que aparezcan.

**4. Coordinación de Git — rama `codex/next-15-5-26` en Windows:** ver instrucciones dadas a Alex en el chat (fuera de este documento, es un procedimiento de una sola vez) — la rama nunca se pusheó a GitHub (`git ls-remote`/`git branch -a` contra `origin` confirmado desde este entorno: solo existe `main` en el remoto) y sus cambios sin commit (fijar `next` a `15.5.26` exacto) quedan superados por lo ya mergeado en `main` (`40a95f5` en adelante, con el mismo bump más toda la validación y documentación). Sin trabajo único que rescatar.

**5. Procedimiento de acceso a `admin@chainpulse.test` — sin cambiar roles a mano ni correr un seed pesado:** `npm run prisma:seed` ejecuta únicamente `prisma/seed.ts` (confirmado en `package.json#prisma.seed` — NO dispara `seed:cuestionario-v2` ni `seed:definiciones-kpi`, que son comandos aparte). Es completamente idempotente: `empresa.upsert`/`usuario.upsert` con `update: {}` — si el admin de prueba ya existe, no lo toca ni duplica nada; si no existe, lo crea con `SEED_ADMIN_PASSWORD` (o `CambiarEstaClave123!` por defecto) y `rol: "ADMINISTRADOR"`. Requiere `SEED_DATABASE_URL` en `.env` (rol `neondb_owner`, nunca `DATABASE_URL`). **Importante:** si el usuario ya existe con una contraseña distinta a la que se ve en el log, el upsert NO la resetea (es un no-op) — el mensaje de consola puede ser engañoso en ese caso. Y por ser ADMINISTRADOR, el primer login lo manda a `/activar-mfa` (RF/A3, `src/middleware.ts`) antes de poder ver `/dashboard` — es el comportamiento esperado, no una falla.

**Validado en este entorno:** typecheck limpio (5 falsos positivos conocidos), lint limpio, test 292/292 sin regresión. `test:integration`/`build`/pruebas con ADMINISTRADOR siguen pendientes de Windows.

## 25. Cierre de la validación de la Sección 24 (2026-09-25)

Alex confirmó en Windows, sobre el commit `bb37870`: coordinación de Git resuelta (rama `codex/next-15-5-26` descartada, alineado a `main`), software probado de punta a punta y "todo bien" — cubre lo que quedaba pendiente de las Secciones 23-24 (`test:integration` post-parche de next, `build`, acceso y pruebas con ADMINISTRADOR incluyendo "Invitar responsable", redirección post-login a `/dashboard`, y sin recurrencia reportada del P2028). Con esto:

- [x] R2 (cliente real, cifrado de aplicación, script de prueba de 10 pasos) — cerrado.
- [x] Next.js 15.5.26 — cerrado, validado de punta a punta.
- [x] Correcciones de UI/auth de las Secciones 22-24 (código de país en teléfono, botón volver al inicio, ocultar "Invitar responsable" a RESPONSABLE, `AUTH_URL` documentada, redirección post-login) — cerradas.
- Las 8 vulnerabilidades de dependencias de la Sección 21 (2 pendientes reales: `deepmerge-ts`/Prisma CLI y `vitest`/`@vitest/mocker`, ambas sin arreglo estable/sin major) siguen en observación con su fecha de próxima revisión, no se reabren acá.

**Corrección de Alex (2026-09-25, mismo día):** los siguientes 4 puntos NO deben presentarse como resueltos/cerrados a partir de un "todo bien" informal — quedan registrados aquí como pendientes, separados de lo que sí tiene evidencia específica documentada arriba:

- [ ] Pruebas con ADMINISTRADOR (acceso, "Invitar responsable", flujo completo) — Alex las corrió, pero sin un detalle paso a paso documentado (a diferencia de las 10 pruebas de R2, sección 20260924). Pendiente de un registro con el mismo nivel de detalle antes de darlas por cerradas.
- [ ] `test:integration` posterior al parche de `next` 15.5.26 — sin salida de comando registrada en este documento. Pendiente.
- [ ] Seguimiento del P2028 — la Sección 24 dejó explícitamente condicionado ("sin recurrencia reportada" no es lo mismo que "no puede recurrir"): sigue en observación, no se toca `TX_OPTIONS_NEON` sin un caso reproducido con Neon ya despierta.
- [ ] Evaluación de las 8 vulnerabilidades y advertencias del build — la Sección 21 documenta 3 cadenas con GHSA/CVE y 2 pendientes reales (`deepmerge-ts`/Prisma CLI, `vitest`), correctamente NO cerradas; esta línea es solo para dejar explícito que "todo bien" del usuario no las reabre ni las cierra — siguen exactamente como las dejó la Sección 21.3, con sus fechas de revisión propias.

**Siguiente tarea (ya acordada, "después del bloque R2": Alex, 2026-09-24) — Incremento 4 Bloque B, importador CSV.** Corrección sobre lo que se iba a escribir acá: el sanitizador de inyección de fórmulas CSV **ya existe** (`src/domain/sanitizacionCsv.ts`, revisado y corregido por Alex el 2026-09-24, 14 pruebas en `sanitizacionCsv.test.ts`) — no es el punto de partida, ya está hecho, igual que los límites de archivo/filas (`src/domain/limitesImportacionCsv.ts`) y todo el bloque de storage/cifrado de R2 (Secciones anteriores). Lo que falta construir es la orquestación real: parseo + mapeo de columnas, el contrato de escritura contra `ObservacionKpi`/`ObservacionCobertura`, los Route Handlers, el job de `pg-boss` y la UI de 3 niveles — nada de eso tiene código todavía (`src/infra/kpis/` no existe). Ver discusión con Alex en el chat sobre el orden concreto.
