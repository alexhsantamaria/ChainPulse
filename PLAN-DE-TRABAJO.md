...
**Script de prueba manual contra R2 dev, ya escrito (pedido de Alex 2026-09-24):** `npm run r2:probar` (`scripts/probarR2.ts`) — cifra un CSV ficticio, lo sube a R2, lo descarga, lo descifra y compara byte a byte contra el original; después intenta descifrar el mismo objeto declarando una empresa distinta (debe lanzar por AAD — si no lanza, el script termina con error, es un fallo real de aislamiento) y borra el objeto de prueba siempre, en un `finally`. Se niega a correr si `R2_BUCKET_NAME` no contiene "dev" en el nombre (salvo `--forzar` explícito) — única protección real contra correrlo sin querer contra producción. Nunca imprime credenciales ni la clave de cifrado, solo el nombre del bucket (no es secreto) y resultados OK/FALLO por paso. Corrélo apenas tengas el bucket de desarrollo y sus credenciales cargadas en `.env` — es la validación de punta a punta que falta antes de dar por cerrado el cliente de R2 real.
