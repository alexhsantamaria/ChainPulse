...
- [ ] **Paso 2 — pendiente:** página pública `/invitacion-cadena/responder?token=...` (sin login) con las preguntas según el alcance de la invitación (cadena completa: prioridad + nodo crítico; conexión puntual: las otras 4), guarda `RespuestaCadena` (upsert manual, no el `upsert` de Prisma: el índice de identidad real son los dos índices únicos parciales de la migración, no expresables como `@@unique` de Prisma por el nullable de `conexionCadenaId`).
