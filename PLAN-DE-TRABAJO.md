...
**Bloque A completado y validado en la Mac (2026-09-20)** — `tsc`/`eslint` limpios sobre los 3 archivos nuevos/tocados (`prisma/schema.prisma`, `src/infra/prisma/tenantScope.ts`, `src/infra/mapa/crearCadenaCompleta.ts`) y sobre las dos pruebas de integración nuevas; `npm run test` sigue en 30/30 archivos, 153/153 tests (sin regresiones, las pruebas nuevas quedan excluidas por nombre `*.integration.test.ts`, mismo criterio que RNF1). Pruebas de integración escritas pero **todavía sin correr contra Neon real** (la Mac no tiene red hacia Neon, ver ADR-0003) — falta que Alex corra `npm run test:integration` en Windows:
