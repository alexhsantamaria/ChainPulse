...
- [ ] **Pendiente:** creación y edición interactiva de nodos/conexiones directamente sobre el canvas (RF34) — el paso que de verdad habilita el mapa (por ahora solo muestra lo que ya exista, creado por otro medio como un dogfooding/seed). Necesita rutas granulares nuevas (`POST /api/cadenas/:id/nodos`, `POST /api/cadenas/:id/conexiones`) que `crearCadenaCompleta()` no cubre (esa función es para la creación atómica del conjunto inicial, no para agregar una pieza a una Cadena ya existente).
- [ ] Indicador visual explícito por dato mostrado (foto puntual / serie temporal, RF35) — nuevo patrón de UI, sin precedente en el dashboard actual (que siempre muestra tendencias ya resueltas server-side, nunca la ambigüedad "esto es un solo punto"). Depende de que existan los datos de RF30-33 (los 5 campos de criticidad + `estadoEvidencia` de una `ConexionCadena`), todavía no expuestos en ningún formulario — mismo criterio que "completar después" ya usado en `Conexion` (RF3).
- [ ] **Pendiente, fuera de alcance de este primer paso:** columnas de posición (`posX`/`posY` o similar) en `Nodo` para persistir dónde el usuario arrastra cada nodo en el canvas — el layout actual es una grilla recalculada en cada carga, sin memoria de una sesión a otra. Necesitaría una migración nueva cuando se construya la edición interactiva.
