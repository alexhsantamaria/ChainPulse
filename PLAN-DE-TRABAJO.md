...
- Rate limiting real (tabla `LimiteTasa`) + job de purga de `huellaOrigen`: **código construido y con pruebas unitarias verdes desde el 2026-09-16** (66/66, `npm run test`); prueba de integración contra Neon real todavía pendiente de la validación en Windows (ver README.md) — este gate se considera cumplido recién cuando esa validación se reporte.
...
1. **[Técnico]** `LimiteTasa` (rate limiting Postgres-nativo) + job de purga de `huellaOrigen` — **código construido, Bloque A, 2026-09-16** (`prisma/schema.prisma`, `src/infra/rateLimit/`, `src/infra/retencion.ts`, `src/infra/jobs/`, `src/app/api/internal/jobs/run`), con pruebas unitarias verdes (66/66). Prueba de integración contra Neon real pendiente de validación en Windows — hasta entonces este gate no se da por completamente cerrado, aunque ya no bloquea seguir con el resto del schema de Incremento 2 a nivel de código (ver Tarea #68).
