...
Con Auth.js, el registro de cuenta (RF1), la activación de MFA y el middleware de protección de rutas implementados (falta validar el middleware en Windows — no debería requerir nada nuevo en `.env`, pero conviene confirmar): conectar el motor a Prisma real, las rutas de Next.js para RF2-RF10 (cuenta completa) y RF11-RF18 (evaluación exprés con el gate macro/detalle), el formulario de RF3 con la agrupación visual que recomendó UX/UI, y las pruebas de integración de aislamiento multi-tenant (RNF1) con datos reales de las dos empresas piloto — antes de tocar el mapa visual (Incremento 2).
