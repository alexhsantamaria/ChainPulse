...
Como el tenant del login se resuelve via `login_lookup()` (funcion `SECURITY DEFINER`, ver seccion principal de este ADR), hubo que agregar la columna `eslabonId` a esa funcion tambien (`prisma/auth_functions.sql`, `CREATE OR REPLACE FUNCTION` — segura de re-correr, no rompe lo existente) para que el login de un responsable la traiga. Este archivo se aplica a mano en el SQL Editor de Neon (mismo mecanismo que `prisma/rls.sql`): **hay que volver a correrlo** antes de que un `RESPONSABLE` pueda responder un cuestionario en Windows, si no `session.user.eslabonId` llega `null` y RF6 no encuentra ninguna conexion asignada.
