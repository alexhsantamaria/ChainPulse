...
**Correccion (2026-09-13) — el middleware no puede importar `auth.ts` completo.** Al probar el middleware en Windows, `npm run dev`/`npm run build` fallaba con `Module not found: Can't resolve '@node-rs/argon2-wasm32-wasi'`. Causa: `src/middleware.ts` importaba `auth` desde `@/auth` (el archivo completo, con el proveedor `Credentials`), y ese archivo arrastra `src/infra/auth/password.ts` (`@node-rs/argon2`, ver addendum de mas arriba) y `loginLookup.ts` (Prisma) — ninguno de los dos corre en el Edge runtime, que ademas intenta resolver la variante `wasm32-wasi` de argon2 al empaquetar el bundle y no la encuentra.

Solucion (patron "Edge compatibility" de la propia documentacion de Auth.js v5): se separo la configuracion en dos archivos. `src/auth.config.ts` (nuevo) tiene solo lo que el middleware necesita -- `session`, `pages`, los callbacks `jwt`/`session` -- sin el proveedor `Credentials` ni ninguna dependencia de Argon2 o Prisma. `src/auth.ts` importa ese config base y le agrega el proveedor `Credentials` completo (usado por las rutas de API y los Server Components, en Node runtime). `src/middleware.ts` ahora construye su propia instancia de `auth()` a partir de `auth.config.ts` directamente (`NextAuth(authConfig).auth`), sin pasar nunca por `auth.ts` -- asi Argon2 y Prisma no entran en el bundle del Edge runtime. El comportamiento no cambia: el middleware sigue decodificando el mismo JWT de sesion, solo que ya no necesita (ni puede) verificar credenciales.

Validado en Windows tras el fix: `npm run dev` arranca sin el error de `argon2-wasm32-wasi`, y sin sesion activa, una peticion directa a `/activar-mfa` redirige correctamente a `/login`. Con esto queda cerrado el addendum: RF1, activacion de MFA y middleware, los tres validados de punta a punta.

## Addendum — `eslabonId` en la sesion, para RF6 (2026-09-13)

RF6 (responder el cuestionario de un ciclo) necesita saber a que eslabon esta asignado un `RESPONSABLE` sin una consulta aparte a la base en cada pagina/ruta. Se agrego `eslabonId` a los cuatro modulos de tipos ampliados en `src/infra/auth/next-auth.d.ts` (`Session.user`, `User`, `JWT` de `next-auth` y de `@auth/core`), se propaga en los callbacks `jwt`/`session` de `src/auth.config.ts`, y se agrega a lo que devuelve `authorize()` en `src/auth.ts`.

Como el tenant del login se resuelve via `login_lookup()` (funcion `SECURITY DEFINER`, ver seccion principal de este ADR), hubo que agregar la columna `eslabonId` a esa funcion tambien (`prisma/auth_functions.sql`) para que el login de un responsable la traiga. Este archivo se aplica a mano en el SQL Editor de Neon (mismo mecanismo que `prisma/rls.sql`).

**Correccion (2026-09-13, mismo dia) — `CREATE OR REPLACE` no alcanza.** El comentario original de este addendum decia que `CREATE OR REPLACE FUNCTION` era seguro de re-correr sin romper nada. Es falso para este caso puntual: Postgres rechaza `CREATE OR REPLACE` cuando cambia el tipo de retorno de una funcion existente (`SQLSTATE 42P13`, "cannot change return type of existing function") — agregar una columna a `RETURNS TABLE` cuenta como cambio de tipo. Se detecto al correr el SQL en Neon (Windows). Solucion: `DROP FUNCTION IF EXISTS login_lookup(text);` antes del `CREATE FUNCTION` (commit `236f3c8`) — `DROP ... IF EXISTS` si es seguro de re-correr, no importa si la funcion ya fue borrada antes.

Validado en Windows de punta a punta: se abrio un ciclo (correo de aviso recibido correctamente por Resend), el `RESPONSABLE` de "Compras" pudo iniciar sesion y `session.user.eslabonId` trajo el eslabon correcto (el cuestionario mostro solo la conexion que le correspondia), respondio, y al cerrar el ciclo la pantalla de resultados mostro valores coherentes (salud 75, riesgo 42, indice de integracion 90, cobertura 100%). Con esto queda cerrado el addendum.
